//
// Thanks to https://github.com/PencilCode/musical.js 
// 
...
makeWaveTable = function(ac) {
    return (function(wavedata) {
      function makePeriodicWave(data) {
        try {
          // Latest API naming.
          return ac.createPeriodicWave(data.real, data.imag);
        } catch (e) { }
        try {
          // Earlier API naming.
          return ac.createWaveTable(data.real, data.imag);
        } catch (e) { }
        return null;
      }
      ...
      for (k in wavedata) {
        ...
        if (d.mult) {
          ...
          for (j = 0; j < ff.length; ++j) {
            ...
            if (wave) { record.push ({freq: ff[j], wave: wave}); }
          }
        }
